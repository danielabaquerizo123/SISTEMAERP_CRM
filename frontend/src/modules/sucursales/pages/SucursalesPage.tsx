...
const initialSucursales: Sucursal[] = [
  { id: '1', nombre: 'Matriz Quito', direccion: 'Av. Amazonas N52-123 y Naciones Unidas', ciudad: 'Quito', provincia: 'Pichincha', telefono: '02-256-7890', encargado: 'Carlos Andrade', tipo: 'sucursal', estado: 'activo', empleados: 45 },
  { id: '2', nombre: 'Sucursal Norte', direccion: 'Av. 10 de Agosto 456 y Eloy Alfaro', ciudad: 'Quito', provincia: 'Pichincha', telefono: '02-267-8901', encargado: 'María Elizalde', tipo: 'sucursal', estado: 'activo', empleados: 28 },
  { id: '3', nombre: 'Sucursal Sur', direccion: 'Av. Quitumbe Ñan 789', ciudad: 'Quito', provincia: 'Pichincha', telefono: '02-278-9012', encargado: 'Pedro Moncayo', tipo: 'sucursal', estado: 'activo', empleados: 32 },
  { id: '4', nombre: 'Sucursal Este', direccion: 'Av. 6 de Diciembre 234 y El Inca', ciudad: 'Quito', provincia: 'Pichincha', telefono: '02-289-0123', encargado: 'Ana Beltrán', tipo: 'sucursal', estado: 'activo', empleados: 22 },
  { id: '5', nombre: 'Sucursal Oeste', direccion: 'Calle La Prensa 567', ciudad: 'Quito', provincia: 'Pichincha', telefono: '02-290-1234', encargado: 'Roberto Ayala', tipo: 'sucursal', estado: 'inactivo', empleados: 18 },
  { id: '6', nombre: 'Centro de Distribución Quito', direccion: 'Km 5.5 Vía a Mitad del Mundo', ciudad: 'Quito', provincia: 'Pichincha', telefono: '02-301-2345', encargado: 'Luis Velasco', tipo: 'centro-distribucion', estado: 'activo', empleados: 15 },
  { id: '7', nombre: 'Bodega Norte', direccion: 'Calle 6 de Diciembre 890', ciudad: 'Quito', provincia: 'Pichincha', telefono: '02-312-3456', encargado: 'Sofía Herrera', tipo: 'bodega', estado: 'activo', empleados: 8 },
  { id: '8', nombre: 'Centro de Distribución Guayaquil', direccion: 'Km 7.5 Vía Daule', ciudad: 'Guayaquil', provincia: 'Guayas', telefono: '04-234-5678', encargado: 'Jorge Plaza', tipo: 'centro-distribucion', estado: 'activo', empleados: 20 },
];

const ciudades = ['Quito', 'Guayaquil'];
const tipos = ['sucursal', 'centro-distribucion', 'bodega'] as const;
const estados = ['activo', 'inactivo'] as const;
const ROWS_PER_PAGE = 10;

const tipoBadge = (tipo: string) => {
  const map: Record<string, { label: string; class: string }> = {
    sucursal: { label: 'Sucursal', class: 'bg-primary-500/10 text-primary-500 border-primary-500/20' },
    'centro-distribucion': { label: 'C. Distribución', class: 'bg-info/10 text-info border-info/20' },
    bodega: { label: 'Bodega', class: 'bg-warning/10 text-warning border-warning/20' },
  };
  return map[tipo] || map.sucursal;
};
...
export default function SucursalesPage() {
  ...
  const [items, setItems] = useState<Sucursal[]>(initialSucursales);
  const [formData, setFormData] = useState({ nombre: '', ciudad: '', direccion: '', telefono: '', encargado: '', tipo: 'sucursal' as Sucursal['tipo'], estado: 'activo' as Sucursal['estado'] });

  const filtered = useMemo(() => {
    return items.filter((s) => {
      if (search && !s.nombre.toLowerCase().includes(search.toLowerCase()) && !s.direccion.toLowerCase().includes(search.toLowerCase())) return false;
      if (ciudadFilter && s.ciudad !== ciudadFilter) return false;
      if (tipoFilter && s.tipo !== tipoFilter) return false;
      return true;
    });
  }, [search, ciudadFilter, tipoFilter, items]);
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      if (editingItem) {
        ...
        setItems(prev => [...prev, { id: crypto.randomUUID(), ...formData, provincia: '', empleados: 0 }]);
      }
      setSaving(false);
      setShowModal(false);
      setEditingItem(null);
      setSuccessMsg(editingItem ? 'Sucursal actualizada exitosamente' : 'Sucursal creada exitosamente');
  ...
  const handleDelete = (id: string) => {
    ...
    setShowDelete(null);
    setSuccessMsg('Sucursal eliminada exitosamente');
    setTimeout(() => setSuccessMsg(''), 3000);
  };

  const openEdit = (item: Sucursal) => {
    setEditingItem(item);
    setFormData({ nombre: item.nombre, ciudad: item.ciudad, direccion: item.direccion, telefono: item.telefono, encargado: item.encargado, tipo: item.tipo, estado: item.estado });
    setShowModal(true);
  };

  const openCreate = () => {
    setEditingItem(null);
    setFormData({ nombre: '', ciudad: '', direccion: '', telefono: '', encargado: '', tipo: 'sucursal', estado: 'activo' });
    setShowModal(true);
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-0.5">Gestión de sucursales, centros de distribución y bodegas</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-gray-50/50">
                ...
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-text-secondary uppercase tracking-wider">Dirección</th>
                ...
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-text-secondary uppercase tracking-wider">Encargado</th>
                ...
                <th className="text-center px-4 py-3.5 text-xs font-semibold text-text-secondary uppercase tracking-wider">Empleados</th>
            ...
            <tbody className="divide-y divide-border">
              {paginated.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5 text-sm text-text-secondary max-w-[200px] truncate">{item.direccion}</td>
                  ...
                  <td className="px-4 py-3.5 text-sm text-text-secondary">{item.encargado}</td>
                  ...
                  <td className="px-4 py-3.5 text-sm font-semibold text-text-primary text-center">{item.empleados}</td>
                  <td className="px-4 py-3.5">
              ...
              {paginated.length === 0 && (
                <tr>
                  <td colSpan={9} className="px-4 py-12 text-center text-text-secondary">
                    ...
                    <p className="text-xs mt-0.5">Intenta ajustar los filtros o crea una nueva sucursal</p>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <motion.div key="modal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                <h3 className="text-lg font-semibold text-text-primary">{editingItem ? 'Editar Sucursal' : 'Nueva Sucursal'}</h3>
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={formData.nombre} onChange={(e) => setFormData({...formData, nombre: e.target.value})} className="w-full px-3 py-2.5 rounded-lg border border-border bg-white text-sm text-text-primary outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all" placeholder="Nombre de la sucursal" />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-1.5">Encargado</label>
                    <input value={formData.encargado} onChange={(e) => setFormData({...formData, encargado: e.target.value})} className="w-full px-3 py-2.5 rounded-lg border border-border bg-white text-sm text-text-primary outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all" placeholder="Nombre del encargado" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-1.5">Tipo</label>
                    <select value={formData.tipo} onChange={(e) => setFormData({...formData, tipo: e.target.value as Sucursal['tipo']})} className="w-full px-3 py-2.5 rounded-lg border border-border bg-white text-sm text-text-primary outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all">
                  ...
                  <div>
                    ...
                    <select value={formData.estado} onChange={(e) => setFormData({...formData, estado: e.target.value as Sucursal['estado']})} className="w-full px-3 py-2.5 rounded-lg border border-border bg-white text-sm text-text-primary outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all">
              ...
              <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border bg-gray-50/50">
                ...
                <button onClick={handleSave} disabled={saving} className="px-4 py-2.5 rounded-lg bg-primary-500 text-white text-sm font-semibold hover:bg-primary-600 disabled:opacity-60 disabled:cursor-not-allowed transition-all flex items-center gap-2">
                  ...
                  {saving ? 'Guardando...' : editingItem ? 'Actualizar Sucursal' : 'Crear Sucursal'}
                </button>
              </div>
            </motion.div>
          </motion.div>
        ...
        {showDelete && (
          <motion.div key="delete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-text-primary mb-2">¿Eliminar sucursal?</h3>
              <p className="text-sm text-text-secondary mb-6">Esta acción no se puede deshacer. La sucursal será eliminada permanentemente.</p>
