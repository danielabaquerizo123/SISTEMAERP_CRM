...
import { Search, Plus, Eye, Edit2, Trash2, X, CheckCircle, AlertTriangle, Loader2, MessageSquare, Clock, CheckSquare, Star } from 'lucide-react';
import type { Ticket } from '@types/index';

const initialTickets: Ticket[] = [
  { id: '1', numero: 'TK-001', cliente: 'María Fernanda Gómez', asunto: 'Producto caducado en compra', categoria: 'calidad', prioridad: 'alta', fecha: '2026-07-05', usuario: 'Pedro Pérez', estado: 'abierto', ultimaActualizacion: '2026-07-05' },
  { id: '2', numero: 'TK-002', cliente: 'Comercializadora Quito SA', asunto: 'Error en facturación', categoria: 'facturacion', prioridad: 'critica', fecha: '2026-07-04', usuario: 'Lucía Medina', estado: 'en_proceso', ultimaActualizacion: '2026-07-06' },
  { id: '3', numero: 'TK-003', cliente: 'Carlos Andrés López', asunto: 'Consulta sobre devolución', categoria: 'devoluciones', prioridad: 'baja', fecha: '2026-07-03', usuario: 'Pedro Pérez', estado: 'resuelto', ultimaActualizacion: '2026-07-04' },
  { id: '4', numero: 'TK-004', cliente: 'Supermercados del Pacífico SA', asunto: 'Solicitud de cotización mayorista', categoria: 'ventas', prioridad: 'media', fecha: '2026-07-02', usuario: 'Ana Torres', estado: 'pendiente', ultimaActualizacion: '2026-07-03' },
  ...
  { id: '6', numero: 'TK-006', cliente: 'Distribuidora Andina Cía. Ltda.', asunto: 'Reclamo por cobro duplicado', categoria: 'facturacion', prioridad: 'critica', fecha: '2026-06-30', usuario: 'Pedro Pérez', estado: 'en_proceso', ultimaActualizacion: '2026-07-01' },
...
const summaryCards = [
  { title: 'Tickets Abiertos', value: 3, icon: MessageSquare, color: 'text-info' },
  { title: 'En Proceso', value: 2, icon: Clock, color: 'text-warning' },
  { title: 'Resueltos', value: 2, icon: CheckSquare, color: 'text-success' },
  { title: 'Satisfacción Promedio', value: '4.5 / 5.0', icon: Star, color: 'text-yellow-500' },
];
...
function AtencionClientePage() {
  ...
  const paginados = useMemo(() => {
    ...
    return filtrados.slice(start, start + 8);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-2xl font-bold text-text-primary">Atención al Cliente</h1>
          <p className="text-text-secondary mt-1">Gestión de tickets y atención al cliente</p>
        </motion.div>
          ...
          {summaryCards.map((card, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`w-12 h-12 rounded-lg bg-primary-50 flex items-center justify-center ${card.color}`}>
                ...
                <tbody className="divide-y divide-border">
                  {paginados.map(t => (
                    <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-text-secondary">{t.fecha}</td>
          ...
          {filtrados.length > 0 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-border">
              <span className="text-sm text-text-secondary">
                Mostrando {Math.min((currentPage - 1) * 8 + 1, filtrados.length)}-{Math.min(currentPage * 8, filtrados.length)} de {filtrados.length}
              </span>
