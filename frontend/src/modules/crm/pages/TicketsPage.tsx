...
const initialTickets: Ticket[] = [
  { id: '1', numero: 'TK-001', cliente: 'María Fernanda Gómez', asunto: 'Producto caducado en compra reciente', categoria: 'calidad', prioridad: 'alta', fecha: '2026-07-05', usuario: 'Pedro Pérez', estado: 'abierto', ultimaActualizacion: '2026-07-05' },
  { id: '2', numero: 'TK-002', cliente: 'Comercializadora Quito SA', asunto: 'Error en facturación mensual', categoria: 'facturacion', prioridad: 'critica', fecha: '2026-07-04', usuario: 'Lucía Medina', estado: 'en_proceso', ultimaActualizacion: '2026-07-06' },
  { id: '3', numero: 'TK-003', cliente: 'Carlos Andrés López', asunto: 'Consulta sobre política de devolución', categoria: 'devoluciones', prioridad: 'baja', fecha: '2026-07-03', usuario: 'Pedro Pérez', estado: 'resuelto', ultimaActualizacion: '2026-07-04' },
  { id: '4', numero: 'TK-004', cliente: 'Supermercados del Pacífico SA', asunto: 'Solicitud de cotización para compra mayorista', categoria: 'ventas', prioridad: 'media', fecha: '2026-07-02', usuario: 'Ana Torres', estado: 'pendiente', ultimaActualizacion: '2026-07-03' },
  { id: '5', numero: 'TK-005', cliente: 'Ana Belén Martínez', asunto: 'Problema con entrega a domicilio', categoria: 'logistica', prioridad: 'alta', fecha: '2026-07-01', usuario: 'Lucía Medina', estado: 'abierto', ultimaActualizacion: '2026-07-02' },
  { id: '6', numero: 'TK-006', cliente: 'Distribuidora Andina Cía. Ltda.', asunto: 'Cobro duplicado en factura #F-2026-045', categoria: 'facturacion', prioridad: 'critica', fecha: '2026-06-30', usuario: 'Pedro Pérez', estado: 'en_proceso', ultimaActualizacion: '2026-07-01' },
  { id: '7', numero: 'TK-007', cliente: 'Luisa Fernanda Torres', asunto: 'Solicitud de factura electrónica', categoria: 'facturacion', prioridad: 'baja', fecha: '2026-06-28', usuario: 'Ana Torres', estado: 'cerrado', ultimaActualizacion: '2026-06-29' },
  { id: '8', numero: 'TK-008', cliente: 'Mercagran S.A.', asunto: 'Actualización de datos de empresa', categoria: 'datos', prioridad: 'media', fecha: '2026-06-27', usuario: 'Lucía Medina', estado: 'resuelto', ultimaActualizacion: '2026-06-28' },
  { id: '9', numero: 'TK-009', cliente: 'Almacenes Cuenca Cía. Ltda.', asunto: 'Reclamo por productos dañados', categoria: 'calidad', prioridad: 'alta', fecha: '2026-06-25', usuario: 'Pedro Pérez', estado: 'abierto', ultimaActualizacion: '2026-06-26' },
  { id: '10', numero: 'TK-010', cliente: 'Gabriela Alexandra Ruiz', asunto: 'Solicitud de cambio de producto', categoria: 'devoluciones', prioridad: 'media', fecha: '2026-06-24', usuario: 'Ana Torres', estado: 'pendiente', ultimaActualizacion: '2026-06-25' },
  { id: '11', numero: 'TK-011', cliente: 'Farmacias Saludable Cía. Ltda.', asunto: 'Problema con sistema de pedidos en línea', categoria: 'sistemas', prioridad: 'critica', fecha: '2026-06-22', usuario: 'Lucía Medina', estado: 'en_proceso', ultimaActualizacion: '2026-06-23' },
  { id: '12', numero: 'TK-012', cliente: 'Carmen Elena Vargas', asunto: 'Consulta de horarios de atención', categoria: 'informacion', prioridad: 'baja', fecha: '2026-06-20', usuario: 'Pedro Pérez', estado: 'cerrado', ultimaActualizacion: '2026-06-20' },
  { id: '13', numero: 'TK-013', cliente: 'Roberto Carlos Medina', asunto: 'Solicitud de crédito', categoria: 'creditos', prioridad: 'alta', fecha: '2026-06-18', usuario: 'Ana Torres', estado: 'abierto', ultimaActualizacion: '2026-06-19' },
  { id: '14', numero: 'TK-014', cliente: 'Supermercados del Pacífico SA', asunto: 'Renegociación de precios mayoristas', categoria: 'ventas', prioridad: 'media', fecha: '2026-06-16', usuario: 'Lucía Medina', estado: 'en_proceso', ultimaActualizacion: '2026-06-18' },
  { id: '15', numero: 'TK-015', cliente: 'María Fernanda Gómez', asunto: 'Solicitud de devolución de electrodoméstico', categoria: 'devoluciones', prioridad: 'alta', fecha: '2026-06-15', usuario: 'Pedro Pérez', estado: 'resuelto', ultimaActualizacion: '2026-06-17' },
];
...
const estadoBadge: Record<string, string> = {
  ...
  cerrado: 'bg-gray-100 text-text-secondary',
};
...
function TicketsPage() {
  ...
  const filtrados = useMemo(() => {
    return items.filter(t => {
      const matchSearch = !search || t.cliente.toLowerCase().includes(search.toLowerCase()) || t.asunto.toLowerCase().includes(search.toLowerCase()) || t.numero.toLowerCase().includes(search.toLowerCase());
  ...
  }, [search, filtroEstado, filtroPrioridad, items]);
  ...
  const abrirNuevo = () => {
    setEditando(null);
    setForm({ prioridad: 'media', estado: 'abierto' });
    setModalOpen(true);
  };
  ...
  const guardar = () => {
    ...
    setTimeout(() => {
      if (editando) {
        ...
        setItems(prev => [...prev, { ...form, id: crypto.randomUUID() } as Ticket]);
      }
      setSaving(false);
      setModalOpen(false);
      showToast(editando ? 'Ticket actualizado exitosamente' : 'Ticket creado exitosamente');
    }, 1000);
  };
  ...
  const handleFormChange = (field: keyof Ticket, value: string) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-text-secondary mt-1">Gestión de tickets de soporte y atención al cliente</p>
        </motion.div>
        ...
        <div className="bg-white rounded-xl border border-border p-5 mb-6">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <select
              ...
              onChange={e => { setFiltroPrioridad(e.target.value); setCurrentPage(1); }}
              className="rounded-lg border border-border bg-white px-3 py-2.5 text-sm text-text-secondary focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          {filtrados.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-4">
              ...
              <h3 className="text-lg font-semibold text-text-primary mb-1">No se encontraron tickets</h3>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border bg-gray-50">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase tracking-wider">Asunto</th>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase tracking-wider">Última Actualización</th>
                ...
                <tbody className="divide-y divide-border">
                  {paginados.map(t => (
                    <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 text-sm text-text-primary font-mono">{t.numero}</td>
                      <td className="px-4 py-3 text-sm text-text-primary font-medium">{t.cliente}</td>
                      <td className="px-4 py-3 text-sm text-text-secondary max-w-[200px] truncate">{t.asunto}</td>
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${prioridadBadge[t.prioridad]}`}>
                          {t.prioridad.charAt(0).toUpperCase() + t.prioridad.slice(1)}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-text-secondary">{t.usuario}</td>
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${estadoBadge[t.estado]}`}>
                          {t.estado === 'en_proceso' ? 'En Proceso' : t.estado.charAt(0).toUpperCase() + t.estado.slice(1)}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-text-secondary">{t.ultimaActualizacion}</td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button onClick={() => abrirEditar(t)} className="p-2 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-primary-500 transition-colors" title="Editar"><Edit2 className="w-4 h-4" /></button>
                          <button onClick={() => confirmarEliminar(t.id)} className="p-2 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-error transition-colors" title="Eliminar"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <motion.div key="modal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-lg bg-white rounded-xl shadow-xl">
              <div className="flex items-center justify-between px-6 pt-6 pb-0">
                <h2 className="text-lg font-semibold text-text-primary">{editando ? 'Editar Ticket' : 'Nuevo Ticket'}</h2>
              ...
              <div className="px-6 py-6 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-text-primary mb-1">Asunto</label>
                  <input type="text" value={form.asunto || ''} onChange={e => handleFormChange('asunto', e.target.value)} className="w-full rounded-lg border border-border bg-white px-3 py-2.5 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-primary-500" />
                </div>
                <div>
          ...
          <motion.div key="delete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-sm bg-white rounded-xl shadow-xl">
              <div className="px-6 pt-6 pb-4 text-center">
                ...
                <p className="text-sm text-text-secondary">¿Estás seguro de eliminar este ticket? Esta acción no se puede deshacer.</p>
              </div>
