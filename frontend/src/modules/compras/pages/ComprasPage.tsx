...
import type { Compra } from '@types/index';

const initialCompras: Compra[] = [
  { id: '1', numero: 'C-2024-001', proveedor: 'Distribuidora Nacional S.A.', ruc: '1790012345001', fecha: '2026-07-05', items: 15, subtotal: 1250.00, impuesto: 150.00, total: 1400.00, metodoPago: 'Transferencia', sucursal: 'Matriz Quito', estado: 'recibida' },
  { id: '2', numero: 'C-2024-002', proveedor: 'Alimentos del Ecuador Cía. Ltda.', ruc: '1790023456001', fecha: '2026-07-04', items: 8, subtotal: 890.50, impuesto: 106.86, total: 997.36, metodoPago: 'Credito', sucursal: 'Sucursal Norte', estado: 'pendiente' },
  { id: '3', numero: 'C-2024-003', proveedor: 'Lácteos del Valle', ruc: '1790034567001', fecha: '2026-07-03', items: 22, subtotal: 2340.00, impuesto: 280.80, total: 2620.80, metodoPago: 'Transferencia', sucursal: 'Matriz Quito', estado: 'recibida' },
  { id: '4', numero: 'C-2024-004', proveedor: 'Carnes Selectas S.A.', ruc: '1790045678001', fecha: '2026-07-02', items: 6, subtotal: 1560.00, impuesto: 187.20, total: 1747.20, metodoPago: 'Efectivo', sucursal: 'Sucursal Sur', estado: 'anulada' },
  { id: '5', numero: 'C-2024-005', proveedor: 'Bebidas del Pacífico', ruc: '1790056789001', fecha: '2026-07-02', items: 30, subtotal: 4800.00, impuesto: 576.00, total: 5376.00, metodoPago: 'Transferencia', sucursal: 'Matriz Quito', estado: 'recibida' },
  { id: '6', numero: 'C-2024-006', proveedor: 'Distribuidora Nacional S.A.', ruc: '1790012345001', fecha: '2026-07-01', items: 12, subtotal: 980.00, impuesto: 117.60, total: 1097.60, metodoPago: 'Credito', sucursal: 'Sucursal Este', estado: 'pendiente' },
  { id: '7', numero: 'C-2024-007', proveedor: ' limpieza Total Cía.', ruc: '1790067890001', fecha: '2026-06-30', items: 10, subtotal: 675.00, impuesto: 81.00, total: 756.00, metodoPago: 'Transferencia', sucursal: 'Matriz Quito', estado: 'recibida' },
  { id: '8', numero: 'C-2024-008', proveedor: 'Frutas y Verduras Fresh', ruc: '1790078901001', fecha: '2026-06-29', items: 5, subtotal: 420.00, impuesto: 50.40, total: 470.40, metodoPago: 'Efectivo', sucursal: 'Sucursal Oeste', estado: 'recibida' },
  { id: '9', numero: 'C-2024-009', proveedor: 'Alimentos del Ecuador Cía. Ltda.', ruc: '1790023456001', fecha: '2026-06-28', items: 18, subtotal: 1950.00, impuesto: 234.00, total: 2184.00, metodoPago: 'Credito', sucursal: 'Sucursal Norte', estado: 'pendiente' },
  { id: '10', numero: 'C-2024-010', proveedor: 'Panificadora Moderna', ruc: '1790089012001', fecha: '2026-06-27', items: 7, subtotal: 340.00, impuesto: 40.80, total: 380.80, metodoPago: 'Efectivo', sucursal: 'Sucursal Sur', estado: 'anulada' },
];

const estados = ['recibida', 'pendiente', 'anulada'] as const;
...
export default function ComprasPage() {
  ...
  const [items, setItems] = useState<Compra[]>(initialCompras);
  const [formData, setFormData] = useState({ numero: '', proveedor: '', ruc: '', fecha: '', subtotal: 0, impuesto: 0, metodoPago: '', sucursal: '' });

  const filtered = useMemo(() => {
    return items.filter((c) => {
      if (search && !c.numero.toLowerCase().includes(search.toLowerCase()) && !c.proveedor.toLowerCase().includes(search.toLowerCase())) return false;
      if (estadoFilter && c.estado !== estadoFilter) return false;
      if (sucursalFilter && c.sucursal !== sucursalFilter) return false;
      return true;
    });
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      if (editingItem) {
        ...
        setItems(prev => [...prev, { id: crypto.randomUUID(), ...formData, items: 0, total: Number(formData.subtotal) + Number(formData.impuesto), estado: 'pendiente' }]);
      }
      setSaving(false);
      setShowModal(false);
      setEditingItem(null);
      setSuccessMsg(editingItem ? 'Compra actualizada exitosamente' : 'Compra creada exitosamente');
  ...
  const handleDelete = (id: string) => {
    ...
    setShowDelete(null);
    setSuccessMsg('Compra eliminada exitosamente');
    setTimeout(() => setSuccessMsg(''), 3000);
  };

  const openEdit = (item: Compra) => {
    setEditingItem(item);
    setFormData({ numero: item.numero, proveedor: item.proveedor, ruc: item.ruc, fecha: item.fecha, subtotal: item.subtotal, impuesto: item.impuesto, metodoPago: item.metodoPago, sucursal: item.sucursal });
    setShowModal(true);
  };

  const openCreate = () => {
    setEditingItem(null);
    setFormData({ numero: '', proveedor: '', ruc: '', fecha: '', subtotal: 0, impuesto: 0, metodoPago: '', sucursal: '' });
    setShowModal(true);
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-0.5">Gestión de compras y abastecimiento</p>
        </div>
                ...
                >
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-text-primary">{item.proveedor}</p>
                        <p className="text-xs text-text-secondary">{item.ruc}</p>
                      </div>
                    </div>
                  </td>
              ...
              {paginated.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-text-secondary">
                    ...
                    <p className="text-xs mt-0.5">Intenta ajustar los filtros o crea una nueva compra</p>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <motion.div key="modal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                <h3 className="text-lg font-semibold text-text-primary">{editingItem ? 'Editar Compra' : 'Nueva Compra'}</h3>
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={formData.numero} onChange={(e) => setFormData({...formData, numero: e.target.value})} className="w-full px-3 py-2.5 rounded-lg border border-border bg-white text-sm text-text-primary outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all" placeholder="Ej: C-2024-XXX" />
                  </div>
                  <div>
              ...
              <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border bg-gray-50/50">
                ...
                <button onClick={handleSave} disabled={saving} className="px-4 py-2.5 rounded-lg bg-primary-500 text-white text-sm font-semibold hover:bg-primary-600 disabled:opacity-60 disabled:cursor-not-allowed transition-all flex items-center gap-2">
                  ...
                  {saving ? 'Guardando...' : editingItem ? 'Actualizar Compra' : 'Crear Compra'}
                </button>
              </div>
            </motion.div>
          </motion.div>
        ...
        {showDelete && (
          <motion.div key="delete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-text-primary mb-2">¿Eliminar compra?</h3>
              <p className="text-sm text-text-secondary mb-6">Esta acción no se puede deshacer. La compra será eliminada permanentemente.</p>
