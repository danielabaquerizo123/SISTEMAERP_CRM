...
export default function Button({ variant = 'primary', size = 'md', loading, isLoading, icon, children, className = '', disabled, ...props }: ButtonProps) {
  ...
  return (
    ...
    >
      {isBusy ? (
        ...
        <span className="w-4 h-4">{React.createElement(icon, { className: "w-4 h-4" })}</span>
      ) : null}
      {children}
    </button>
  );
